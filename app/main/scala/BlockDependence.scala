...
object BlockDependence extends App {
  ...
  class MyDSLProgram extends DFT_Exp {
    ...
    def globalit(s: SMix, f: DMix => Rep[Int]) = {
      val exp = exposeDMix(s)
      val intexp: ExposeRep[Rep[Int]] = exposeRepFromRep[Int]
      doGlobalLambda(f,true)(exp,intexp)
    }
    ...
    def myprog(s: SMix): (DMix => Rep[Int]) = {
      val outer: (DMix => Rep[Int]) = (z: DMix) => {
        ...
        def f (s: SMix): (DMix => Rep[Int]) = (inz: DMix) => {
          inz.c
        }
        def g(s: SMix): (DMix => Rep[Int]) = (inz: DMix) => {
          val hf = globalit(s,f(s))
          hf (inz)
          //inz.c * hf(inz)
          //inz.c * inz.c
        }
        val gf = globalit(s,f(s))
        val hf = globalit(s,g(s))
        val t0 = gf(dyn_info)
        val t1 = hf(dyn_info)
        t0*t1
